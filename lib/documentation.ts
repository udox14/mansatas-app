...
const FEATURE_DOCS: Record<string, { summary: string; sections: string[] }> = {
  ...
  'rekap-absensi': {
    ...
    sections: [
      ...
      '- Status akhir menggabungkan input Absensi Siswa dan koreksi harian wali kelas; detail input guru tetap tersimpan terpisah.',
    ],
  },
  'keterangan-absensi': {
    summary: 'Melihat riwayat dan menetapkan koreksi status akhir harian siswa tanpa mengubah detail absensi asli guru.',
    sections: [
      '## Fungsi yang tersedia',
      '- Melihat ringkasan status harian berdasarkan seluruh sesi absensi guru.',
      '- Menetapkan koreksi Hadir, Sakit, Izin, atau Alfa sebagai status akhir harian.',
      '- Tetap menampilkan detail sesi guru sebagai dasar keputusan wali kelas.',
      '## Tombol dan aksi',
      '- Pilihan status menetapkan koreksi wali kelas.',
      '- Ikuti Guru menghapus koreksi sehingga status akhir kembali mengikuti ringkasan guru.',
      '- Simpan menyimpan keputusan dan catatan wali kelas.',
      '## Filter dan data',
      '- Filter tanggal, kelas, siswa, dan status absensi membantu menemukan data.',
      '- Koreksi muncul pada status akhir, rekap, dan detail siswa, tetapi tidak mengubah tabel absensi sesi guru.',
    ],
  },
  agenda: {
