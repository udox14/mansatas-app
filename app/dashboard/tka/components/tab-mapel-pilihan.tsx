    ...
    <div className="space-y-4">
      ...
      {dirtyRows.length > 0 && (
        <div className="text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-lg px-3 py-2">
          {dirtyRows.length} perubahan belum disimpan — klik <strong>Simpan</strong> untuk menyimpan ke database.
        </div>
      )}
      ...
      {!loading && rows.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl">
          ...
          {mode === 'idle' && (
            <>
              <p className="text-slate-500 font-medium text-sm">Belum ada kelas dipilih</p>
              <p className="text-slate-400 text-xs mt-1">Pilih kelas dari dropdown, atau cari nama siswa</p>
            </>
          )}
          {mode === 'kelas' && (
            <>
              <p className="text-slate-500 font-medium text-sm">Tidak ada siswa aktif di kelas ini</p>
              <p className="text-slate-400 text-xs mt-1">Pastikan siswa sudah diplot ke kelas yang benar</p>
            </>
          )}
          {mode === 'search' && (
            <>
              <p className="text-slate-500 font-medium text-sm">Siswa tidak ditemukan</p>
              <p className="text-slate-400 text-xs mt-1">Coba kata kunci yang berbeda</p>
            </>
          )}
        </div>
      )}
      ...
      {!loading && rows.length > 0 && (
        <div className="rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
                  ...
                  <th className="text-left text-xs font-semibold text-slate-500 dark:text-slate-400 px-4 py-2.5 w-32">NISN</th>
                  <th className="text-left text-xs font-semibold text-slate-500 dark:text-slate-400 px-4 py-2.5 min-w-[210px]">Mapel Pilihan 1</th>
                  <th className="text-left text-xs font-semibold text-slate-500 dark:text-slate-400 px-4 py-2.5 min-w-[210px]">Mapel Pilihan 2</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((row, idx) => (
          ...
          <div className="px-4 py-2 bg-slate-50 dark:bg-slate-800/30 border-t border-slate-200 dark:border-slate-700 flex items-center justify-between text-xs text-slate-400">
            ...
            {dirtyRows.length > 0 && (
              <span className="text-amber-500">{dirtyRows.length} belum disimpan</span>
            )}
          </div>
        </div>
      )}
