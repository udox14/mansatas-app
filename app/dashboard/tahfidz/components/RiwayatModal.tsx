    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-xl max-h-[85vh] flex flex-col">
        <DialogHeader>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">Siswa: {siswaNama}</p>
        </DialogHeader>
          ...
          {riwayat.length === 0 ? (
            ...
            <div className="space-y-4">
              {riwayat.map((log) => {
                ...
                return (
                  <div key={log.id} className="p-3 border rounded-lg bg-slate-50 dark:bg-slate-800 relative">
                    ...
                    <div>
                      ...
                      <span className="text-slate-500 dark:text-slate-400 text-sm ml-2">Juz {log.juz}</span>
                    </div>
                    <div className="text-sm text-emerald-600 font-medium mt-1">
                      Ayat yang disetorkan: {formatAyatRange(ayatList)} ({ayatList.length} ayat)
                    </div>
                    {log.keterangan && (
                      <div className="text-sm text-slate-600 dark:text-slate-400 mt-2 bg-white dark:bg-slate-900 p-2 rounded border border-slate-100 dark:border-slate-800">
                        "{log.keterangan}"
                      </div>
                    )}
                  </div>
