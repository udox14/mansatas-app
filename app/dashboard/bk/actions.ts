...
// ============================================================
// 1. KELAS BINAAN — diatur oleh super_admin di manajemen kelas
// ============================================================

// Ambil semua kelas binaan seorang guru BK (untuk validasi akses)
export async function getKelasBinaanGurubk(guru_bk_id: string) {
  ...
  `).bind(guru_bk_id).all<any>()
  return res.results || []
}

// Set kelas binaan untuk satu guru BK (replace semua)
export async function setKelasBinaanBK(guru_bk_id: string, kelas_ids: string[]) {
  const db = await getDB()
  try {
    // Hapus semua binaan lama guru ini
    await db.prepare('DELETE FROM kelas_binaan_bk WHERE guru_bk_id = ?').bind(guru_bk_id).run()

    if (kelas_ids.length > 0) {
      // Insert baru — chunk 15 (15×3 = 45 vars, aman)
      const CHUNK = 15
...
export async function tambahTopikBK(bidang: BidangBK, nama: string, guru_bk_id: string) {
  ...
  const db = await getDB()
  const result = await dbInsert(db, 'bk_topik', {
    bidang,
    nama: nama.trim(),
    created_by: guru_bk_id,
  })
  if (result.error) {
    return { error: result.error.includes('UNIQUE') ? 'Topik ini sudah ada.' : result.error }
  }
...
// Search siswa dari kelas binaan — lazy load
export async function searchSiswaBinaan(
  guru_bk_id: string,
  query: string,
  ...
  // Ambil kelas binaan dulu (1 query)
  ...
  // Query siswa dari kelas binaan yang match nama/NISN
  ...
  const res = await db.prepare(`
    ...
    WHERE s.kelas_id IN (${placeholders})
      AND s.status = 'aktif'
      AND (
        LOWER(s.nama_lengkap) LIKE LOWER(?)
        OR s.nisn LIKE ?
      )
...
// Ambil semua rekaman seorang siswa (lazy load saat klik siswa)
...
export async function tambahRekamanBK(payload: {
  ...
  // Validasi akses: siswa harus dari kelas binaan guru BK ini
  const binaanRes = await db.prepare(
    'SELECT kelas_id FROM kelas_binaan_bk WHERE guru_bk_id = ?'
  ).bind(payload.guru_bk_id).all<{ kelas_id: string }>()
  const kelasIds = new Set((binaanRes.results || []).map(r => r.kelas_id))

  const siswa = await db.prepare('SELECT kelas_id FROM siswa WHERE id = ?')
    .bind(payload.siswa_id).first<{ kelas_id: string }>()
  if (!siswa || !kelasIds.has(siswa.kelas_id)) {
    return { error: 'Siswa ini bukan dari kelas binaan Anda.' }
  }

  const result = await dbInsert(db, 'bk_rekaman', {
    siswa_id: payload.siswa_id,
    guru_bk_id: payload.guru_bk_id,
    tahun_ajaran_id: payload.tahun_ajaran_id,
    bidang: payload.bidang,
    topik_id: payload.topik_id || null,
...
// Edit rekaman (deskripsi, topik, tindak lanjut)
export async function editRekamanBK(id: string, payload: {
  ...
  const db = await getDB()
  const result = await dbUpdate(db, 'bk_rekaman', {
    ...payload,
...
// Tambah sesi penanganan ke rekaman
export async function tambahSesiPenanganan(
  rekaman_id: string,
  tipe: TipePenanganan,
  tanggal: string,
  catatan: string
  ...
  const db = await getDB()
  const row = await db.prepare('SELECT penanganan FROM bk_rekaman WHERE id = ?')
    .bind(rekaman_id).first<{ penanganan: string }>()
  ...
  await db.prepare(
    "UPDATE bk_rekaman SET penanganan = ?, updated_at = datetime('now') WHERE id = ?"
  ).bind(JSON.stringify(sesiList), rekaman_id).run()

  revalidatePath('/dashboard/bk')
  return { success: 'Sesi penanganan ditambahkan.' }
}

// Hapus satu sesi penanganan
...
export async function getInitialDataBK(guru_bk_id: string, is_admin: boolean) {
  ...
  // Kelas binaan — kalau admin ambil semua kelas, kalau guru BK ambil binaannya saja
