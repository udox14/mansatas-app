...
import { getSanksiList, type SanksiConfig } from './actions'
...
async function KedisiplinanDataFetcher({ currentUser, taAktifId, sanksiList, lifetimePoin }: {
  ...
  const formattedKasus = (kasusResult.results || []).map((p: any) => ({
    id: p.id, tanggal: p.tanggal, jam_input: p.jam_input ?? '', keterangan: p.keterangan, foto_url: p.foto_url,
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-500 pb-12">
      <PageHeader
        ...
        description="Catat pelanggaran siswa dan kelola kamus poin tata tertib."
      >
