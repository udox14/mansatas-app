  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { label: 'Total Kasus', value: ringkasan.total_kasus, icon: BarChart3, color: 'text-slate-600 dark:text-slate-400', bg: 'bg-slate-100 dark:bg-slate-800/80 dark:bg-slate-800' },
          ...
          <div key={card.label} className={`rounded-xl p-4 border border-slate-200 dark:border-slate-800 dark:border-slate-700 ${card.bg} flex flex-col gap-2`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        {/* Tren Per Bulan */}
        <div className="md:col-span-2 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 dark:border-slate-700 p-4">
          <h3 className="text-sm font-bold text-slate-700 dark:text-slate-300 dark:text-slate-200 mb-4 flex items-center gap-2">
        ...
        {/* Distribusi Kategori */}
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 dark:border-slate-700 p-4">
          <h3 className="text-sm font-bold text-slate-700 dark:text-slate-300 dark:text-slate-200 mb-4 flex items-center gap-2">
                ...
                return (
                  <div key={k.kategori}>
                    <div className="flex justify-between items-center mb-0.5">
                      <span className="text-xs font-semibold text-slate-600 dark:text-slate-400 dark:text-slate-300">{k.kategori}</span>
                      <span className="text-xs text-slate-400 dark:text-slate-500">{k.jumlah}x · {k.total_poin} poin</span>
                    </div>
                    <div className="h-2 rounded-full bg-slate-100 dark:bg-slate-800/80 dark:bg-slate-800 overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 dark:border-slate-700 p-4">
          <h3 className="text-sm font-bold text-slate-700 dark:text-slate-300 dark:text-slate-200 mb-3 flex items-center gap-2">
            ...
            {topPelanggaran.length === 0 ? (
              ...
              <div key={i} className="flex items-center gap-2 py-1">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 dark:text-slate-200 truncate">{p.nama_pelanggaran}</p>
            ...
            {perKelas.length === 0 ? (
              ...
              return (
                <div key={i} className="flex items-center gap-2">
                  <span className="text-[11px] font-bold w-10 text-slate-600 dark:text-slate-400 dark:text-slate-300 shrink-0">{namaKelas}</span>
                  <div className="flex-1 h-2 rounded-full bg-slate-100 dark:bg-slate-800/80 dark:bg-slate-800 overflow-hidden">
      ...
      {/* ── RADAR SISWA ── */}
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 dark:border-slate-700 overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-slate-800 flex-wrap gap-2">
          <h3 className="text-sm font-bold text-slate-700 dark:text-slate-300 dark:text-slate-200 flex items-center gap-2">
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            <button onClick={() => setFilterLevel('all')}
              className={cn('text-[10px] font-bold px-2 py-0.5 rounded-full transition-all',
                filterLevel === 'all' ? 'bg-slate-800 dark:bg-slate-100 text-white dark:text-slate-900' : 'bg-slate-100 dark:bg-slate-800/80 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700')}>
              Semua
            </button>
            {sanksiList.map(s => (
              <button key={s.id} onClick={() => setFilterLevel(s.id)}
                className={cn('text-[10px] font-bold px-2 py-0.5 rounded-full transition-all',
                  filterLevel === s.id ? 'bg-slate-800 dark:bg-slate-100 text-white dark:text-slate-900' : 'bg-slate-100 dark:bg-slate-800/80 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700')}>
                ...
                <div key={s.id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <p className="text-sm font-bold text-slate-800 dark:text-slate-200 dark:text-slate-100 truncate">{s.nama_lengkap}</p>
                  ...
                  <Link href={`/dashboard/siswa/${s.id}`}
                    className="shrink-0 p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800/80 dark:hover:bg-slate-700 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-400 dark:hover:text-slate-300 transition-colors">
        ...
        <>
          <div className="flex justify-end">
            <Button
              ...
              className="h-8 text-xs gap-1.5 border-slate-200 dark:border-slate-800 dark:border-slate-700"
            >
